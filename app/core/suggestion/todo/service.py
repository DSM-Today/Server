...
from app.utils.dao.mysql.cqrs.subject.suggest.todo.query import query_todo_list, query_my_todo_list
...
def query_my_todo(token: str):
    user_id = get_user_id(token)

    return {
        'my_todo_list': query_my_todo_list(user_id)
    }